...
def load_model(model_selection):   
    ...
    if (model_selection == 'Simple NN'):
        ...
        model = SimpleCNN(10)
        try:
            model.load_state_dict(torch.load('model_nn_0501_cnn.pth'))
...
with display:
    ...
    if canvas and canvas.image_data is not None:
        ...
        img_tensor = (torch.tensor(img).float() / (normalization if normalize else 255.0)).permute(2, 0, 1) # [28, 28, 3] -> [3, 28, 28]
...
if predictions is None:
    ...
    with c1:
            st.markdown(f"<div style='display: flex; align-items: baseline; margin-top: 0;'><h1 style='font-size: 100px'>{predictions[0]['class']}</h1><p>({predictions[0]['probability'] * 100 :.3f}%)</p></div>", unsafe_allow_html=True)
    with c2:
        st.text('I\'m seeing other things too...')
        st.text(f"... {predictions[1]['class']} ({predictions[1]['probability'] * 100 :.3f}%)")
        st.text(f"... {predictions[2]['class']} ({predictions[2]['probability'] * 100 :.3f}%)")
        st.text(f"... {predictions[3]['class']} ({predictions[3]['probability'] * 100 :.3f}%)")